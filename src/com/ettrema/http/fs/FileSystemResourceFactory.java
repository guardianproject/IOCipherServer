...
public class FileSystemResourceFactory implements ResourceFactory {
	 ...
	 * 
	 */
	public FileSystemResourceFactory(String sRoot) {
	...
	//	log.debug("root: " + root.getAbsolutePath());
		this.root = root;
		if (!root.exists()) {
			warn("Root folder does not exist: " + root.getAbsolutePath());
		}
		if (!root.isDirectory()) {
			warn("Root exists but is not a directory: " + root.getAbsolutePath());
		}
	}

	public Resource getResource(String host, String url) {
		debug("getResource: host: " + host + " - url:" + url);
	...
	public FsResource resolveFile(String host, File file) {
		FsResource r;
		if (!file.exists()) {
			debug("file not found: " + file.getAbsolutePath());
			return null;
	...
	public void setSecurityManager(SecurityManager securityManager) {
		if (securityManager != null) {
			debug("securityManager: " + securityManager.getClass());
		} else {
			warn("Setting null FsSecurityManager. This WILL cause null pointer exceptions");
		}
